...
function determineUpgrades(npmDep, config) {
  ...
  .reject(version => config.ignoreUnstable && stable.is(changeLogFromVersion) && !stable.is(version))
  // Ignore future versions, unless the current version is marked as future
  .reject(version => config.ignoreFuture && !isFuture(versions[changeLogFromVersion]) && isFuture(versions[version]))
  // Ignore versions newer than "latest", unless current version is newer than the "latest"
  .reject(version => config.respectLatest && isPastLatest(npmDep, version) && !isPastLatest(npmDep, changeLogFromVersion))
  ...
  .forEach(newVersion => {
    ...
    if (!allUpgrades[upgradeKey] || semver.gt(newVersion, allUpgrades[upgradeKey].newVersion)) {
      const type = newVersionMajor > semver.major(changeLogFromVersion) ? 'major' : 'minor';
      const changeLogToVersion = newVersion;
      const automergeEnabled = config.automerge === 'any' || config.automerge === 'minor' && type === 'minor';
  ...
  const rangedUpgrades = _(upgrades).reject(upgrade => upgrade.type === 'pin').map(upgrade => Object.assign(upgrade, { isRange: true })).map(upgrade => {
