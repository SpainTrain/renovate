...
function isScheduledNow(config) {
  ...
  // Massage into array
  const configSchedule = typeof config.schedule === 'string' ? [config.schedule] : config.schedule;
  ...
  const isWithinSchedule = configSchedule.some(scheduleText => {
    ...
    return parsedSchedule.schedules.some(schedule => {
      ...
      if (schedule.d) {
        ...
        const dowMap = [null, 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
        ...
        if (scheduledDays.indexOf(currentDay) === -1) {
          config.logger.debug(`Does not match schedule because ${currentDay} is not in ${scheduledDays}`);
          return false;
        }
      }
      ...
      if (schedule.t_a) {
        ...
        if (currentSeconds < startSeconds) {
          config.logger.debug(`Does not match schedule because ${currentSeconds} is earlier than ${startSeconds}`);
          return false;
        }
      }
      // Check for end time
      if (schedule.t_b) {
        ...
        if (currentSeconds > endSeconds) {
          config.logger.debug(`Does not match schedule because ${currentSeconds} is later than ${endSeconds}`);
          return false;
        }
      }
