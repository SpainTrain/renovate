...
let setNpmrc = (() => {
  var _ref = _asyncToGenerator(function* (config) {
    try {
      let npmrc = null;
      const npmrcContent = yield config.api.getFileContent('.npmrc');
      if (npmrcContent) {
...
let initApis = (() => {
  var _ref2 = _asyncToGenerator(function* (inputConfig) {
    ...
    yield config.api.initRepo(config.repository, config.token, config.endpoint, config.logger);
    // Check for presence of .npmrc in repository
    yield module.exports.setNpmrc(config);
    return config;
  });

  return function initApis(_x2) {
...
let mergeRenovateJson = (() => {
  var _ref3 = _asyncToGenerator(function* (config, branchName) {
    let returnConfig = Object.assign({}, config);
    const renovateJsonContent = yield config.api.getFileContent('renovate.json', branchName);
    ...
    try {
      ...
      let jsonValidationError = jsonValidator.validate(renovateJsonContent, allowDuplicateKeys);
      ...
      jsonValidationError = jsonValidator.validate(renovateJsonContent, allowDuplicateKeys);
      if (jsonValidationError) {
        const error = {
          depName: 'renovate.json',
          message: jsonValidationError
  ...
  return function mergeRenovateJson(_x3, _x4) {
    return _ref3.apply(this, arguments);
  };
})();

let detectPackageFiles = (() => {
  var _ref4 = _asyncToGenerator(function* (input) {
    const config = Object.assign({}, input);
    config.logger.trace({ config }, 'detectPackageFiles');
    config.packageFiles = yield config.api.findFilePaths('package.json');
    ...
    if (config.ignoreNodeModules) {
      ...
      config.packageFiles = config.packageFiles.filter(function (packageFile) {
      ...
      if (skippedPackageFiles.length) {
        ...
        config.logger.debug(`Now have ${config.packageFiles.length} package file(s) after filtering`);
      }
    }
  ...
  return function detectPackageFiles(_x5) {
