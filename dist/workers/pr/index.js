...
let ensurePr = (() => {
  var _ref = _asyncToGenerator(function* (inputConfig, logger, errors, warnings) {
    ...
    const branchStatus = yield config.api.getBranchStatus(branchName, config.requiredStatusChecks);

    // Only create a PR if a branch automerge has failed
    if (config.automergeEnabled && config.automergeType.startsWith('branch')) {
      logger.debug(`Branch is configured for branch automerge, branchStatus is: ${branchStatus}`);
    ...
    for (const upgrade of upgrades) {
      ...
      const logJSON = yield changelogHelper.getChangeLogJSON(upgrade.depName, upgrade.changeLogFromVersion, upgrade.changeLogToVersion, logger);
      // Store changelog markdown for backwards compatibility
      if (logJSON) {
        upgrade.githubName = logJSON.project.github;
        upgrade.releases = [];
        logJSON.versions.forEach(function (version) {
            ...
            release.changes.forEach(function (change) {
              ...
              if (change.message) {
                ...
                if (config.isGitHub && config.privateRepo === true) {
                  const re = /([\s(])#(\d+)([)\s]?)/g;
                  commit.message = commit.message.replace(re, `$1[#$2](${upgrade.repositoryUrl}/issues/$2)$3`);
                }
              }
    ...
    try {
      ...
      const existingPr = yield config.api.getBranchPr(branchName);
      if (existingPr) {
        ...
        yield config.api.updatePr(existingPr.number, prTitle, prBody);
        logger.info(`Updated ${existingPr.displayNumber}`);
        return existingPr;
      }
      logger.debug(`Creating PR for branch ${branchName}`);
      const pr = yield config.api.createPr(branchName, prTitle, prBody);
      if (config.labels.length > 0) {
        yield config.api.addLabels(pr.number, config.labels);
      }
      ...
      if (config.automergeEnabled && config.automergeType === 'pr') {
        ...
        if (config.assignees.length > 0) {
          yield config.api.addAssignees(pr.number, config.assignees);
        }
        if (config.reviewers.length > 0) {
          yield config.api.addReviewers(pr.number, config.reviewers);
        }
      }
  ...
  return function ensurePr(_x, _x2, _x3, _x4) {
    return _ref.apply(this, arguments);
  };
})();

let checkAutoMerge = (() => {
  var _ref2 = _asyncToGenerator(function* (pr, config, logger) {
    ...
    if (config.automergeEnabled && config.automergeType === 'pr') {
      ...
      // Check branch status
      const branchStatus = yield config.api.getBranchStatus(pr.head.ref, config.requiredStatusChecks);
      ...
      yield config.api.mergePr(pr);
    } else {
      logger.debug('No automerge');
    }
  });

  return function checkAutoMerge(_x5, _x6, _x7) {
