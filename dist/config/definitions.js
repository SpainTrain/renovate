...
function template(name, subdir = 'default/') {
  ...
  path.resolve(__dirname, '../config/templates/', subdir, shortName), 'utf8');
...
// Scheduling
{
  ...
  name: 'baseBranch',
  description: 'Base branch to target for Pull Requests. Otherwise default branch is used',
  stage: 'repository',
  type: 'string',
  cli: false,
  env: false,
  ...
  description: 'Skip any package.json files found within node_modules folders',
  type: 'boolean',
  stage: 'repository',
  onboarding: false
}, {
  ...
  stage: 'packageFile',
  type: 'list',
  default: [{ depType: 'dependencies', semanticPrefix: 'fix(deps): ' }, 'devDependencies', 'optionalDependencies']
},
// depType
{
  name: 'ignoreDeps',
...
// Automatic merging
{
  ...
  name: 'automergeType',
  description: 'How to automerge - "branch-merge-commit", "branch-push" or "pr". Branch support is GitHub-only',
  type: 'string',
  default: 'pr',
  onboarding: false
}, {
...
// Yarn Lock Maintenance
{
  ...
  description: 'Location of yarn cache folder to use. Set to empty string to disable',
  stage: 'global',
  type: 'string',
